          ...
          messages: [
            {
              role: "system",
              content:
                "You are a productivity assistant helping break down tasks into actionable steps. Provide 4 clear, concise suggestions for completing the task. Format your response as a simple list with each suggestion on a new line, no numbers or bullets. Keep each suggestion under 10 words if possible.",
            },
            {
              role: "user",
...
export const generateTaskSuggestions = async (
  ...
  try {
    ...
    console.error(
      "Error generating suggestions with AI API, using fallback:",
      error
    );
    return getFallbackSuggestions(taskTitle);
  }
